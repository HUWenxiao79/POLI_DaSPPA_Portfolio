...
d_subset_long <- d_subset |>pivot_longer (cols=starts_with("e"))
```

```{r}
d_subset_wide_year <-
  d_subset_long |>pivot_wider(names_from=year,
                              values_from = value)
```
...
```{r}
d_1945_1922 <- bind_rows(d,d_1945_1983)
d_1945_1922 |>select(year) |>distinct() |>arrange(year)
```
