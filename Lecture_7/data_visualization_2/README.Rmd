...
# for a histogram ---only x, no need for y 
d |> 
  ggplot(aes(x = gdppc)) +
...
# 因为现在是density的图，如果还是7500，下面的图像看不到， need to check the range of the density 自己选一个比较合适的
```
...
`geom_label` and `geom_text` may get you uninformative annotation if the points you want to annotate cluster in a small area of your figure. 可能会重叠
...
[If the data seem to break at least one of the assumptions for Pearson correlation, you may use Spearman correlation (aka. Spearman's $\rho$).]{.underline}
